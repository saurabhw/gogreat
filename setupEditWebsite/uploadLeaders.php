<?php
  include '../includes/autoload.php';
   ...
   $queryfg = "SELECT * FROM distributors WHERE email = '$repEmail'";
   $resultfg = mysqli_query($link, $queryfg)or die ("couldn't execute query fg.".mysqli_error($link));
   $rowFG = mysqli_fetch_assoc($resultfg);
   $scID = $rowFG['setupID'];
   ...
   $conn = mysql_connect("$hostname","$username","$password") or die(mysqli_error($link));
     ...
     while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
      {
        ...
        $password = mysqli_real_escape_string($link, $filesop[4]);
        ...
        if(mysqli_num_rows($resultB) > 0)
        {
	     ...
	     $dealer = addslashes($dealer);
	      ...
	      $query3 = "INSERT INTO Dealers (Dealer, DealerDir, About, FundraiserReasons, FundraiserGoal,  FundraiserStartDate, FundraiserEndDate, Phone, Address1, Address2, City, State, Zip, PaypalEmail, Banner,  setuppersonid, setuppersonid3, setuppersonid2,z signuptype, orgtype, email, website, facebook, twitter, leader_pic, location_pic,  group_team_pic, userName, banner_path, firstPass, clubType)";
		$query3 .= "VALUES('$dealer','$dealerdir','$about','$reason','$goal','$start','$endz', '$phone1', '$address1', '$address2', '$ct','$st','$zp','$payMail','$banner', '$groupID','$userID','$sponsor', '$signupType', '$orgType', '$email','$url','$face', '$twt','$leaderPic', '$locationPic', '$groupPic', '$email','$banner', '$password', '$club')";
  ...
  $queryH = "INSERT INTO orgMembers(Title, orgFName, orgLName, orgEmail, orgPhone,orgRel, fundraiserID, fund_owner_id, repID, isLeader)VALUES('$titlex', '$fname','$lname','$email','$phone', '$role', '$logID','$groupID','$userID','1')";
	$resultH = mysqli_query($link, $queryH)or die ("couldn't execute query h.".mysqli_error($link));
        ...
        else
        {
             ...
             //get group info
              $group_query = "SELECT * FROM Dealers WHERE loginid='$groupID'";
	      $g_result = mysqli_query($link, $group_query)
	      or die ("couldn't execute query group query E.".mysqli_error($link));
	      $group_row = mysqli_fetch_assoc($g_result);
	  
	      $dealer = mysqli_real_escape_string($link, $group_row['Dealer']);
	      $dealerdir = mysqli_real_escape_string($link, addslashes($group_row['DealerDir']));
	      $about = mysqli_real_escape_string($link, $group_row['About']); 
	      $reason = mysqli_real_escape_string($link, $group_row['FundraiserReasons']);
	      $goal = mysqli_real_escape_string($link, $group_row['FundraiserGoal']); 
	      $start = mysqli_real_escape_string($link, $group_row['FundraiserStartDate']); 
	  ...
	  $query2 .= "VALUES('$dealer', '$fname','$lname','$email', '$repID', '$phn', '$payMail','$role', '$dealerdir', '$titlex', '$user_table_id')";
        ...
        */
		...
		$query3 .= " VALUES('$dealer','$dealerdir','$about','$reason','$goal','$start','$endz', '$phone1', '$address1', '$address2', '$ct','$st','$zp','$payMail','$banner',  '$groupID','$userID','$sponsor', '$signupType', '$orgType', '$email','$url','$face', '$twt','$leaderPic', '$locationPic', '$groupPic', '$email','$banner','$pass', '$club')";
		 $queryResult3 = mysqli_query($link, $query3)or die ("couldn't execute query 3.".mysqli_error($link));
		 //get the member
		...
		$logID = $rowL['loginid'];
		  ...
		  //insert into the orgMembers table
		$queryX = "INSERT INTO orgMembers(Title, orgFName, orgLName, orgEmail, orgPhone,orgRel, fundraiserID, fund_owner_id, repID, isLeader, scID)VALUES('$titlex', '$fname','$lname','$email','$phone', '$role', '$logID','$groupID','$userID', '1','$scID')";
