<?php
	  ...
	  $g_result = mysqli_query($link, $group_query)or die ("couldn't execute query group query.".mysql_error($link));
	  ...
	  $user_page = "../membersite.php?group=";
       ...
       //insert Dealer
        $query3 = "INSERT INTO $table3 (Dealer, DealerDir, About, FundraiserReasons, FundraiserGoal, FundraiserStartDate, FundraiserEndDate, Address1, Address2, City, State, Zip, PaypalEmail, Banner, sponsorid, setuppersonid, setuppersonid2, signuptype, orgtype, email, website, facebook, twitter, leader_pic, location_pic,  group_team_pic, userName, banner_path, firstPass)";
        
		$query3 .= " VALUES('$dealer','$dealerdir','$about','$reason','$goal','$start','$endz', '$address1', '$address2', '$ct','$st','$zp','$payMail','$banner', '$sponsor','$club_id','$repID', '$signupType', '$orgType', '$email','$url','$face', '$twt','$leaderPic', '$locationPic', '$groupPic', '$email', '$banner','$pass')";
		...
		if($res1 && $res2 && $res3){
			...
			mysqli_query($link, $query10);
		        ...
		        $queryH = "INSERT INTO orgMembers(Title,orgFName, orgLName, orgEmail, orgPhone,orgRel, fundraiserID, fund_owner_id, repID, isLeader)VALUES('$tt', '$fname','$lname','$email','$phn', '$role', '$logID','$club_id', '$repID', '1')";
	...
	$resultL1 = mysqli_query($link, $leaderInsert1)or die ("couldn't execute Leader Insert 1.".mysql_error($link));
     ...
     */
	  ...
	  else
	  {
	     ...
	     $foundUser = $userRow['userInfoID'];
	      ...
	      $query15 =  "INSERT INTO $table3 (Dealer, DealerDir, About, FundraiserReasons, FundraiserGoal, FundraiserStartDate, FundraiserEndDate, Address1, Address2, City, State, Zip, PaypalEmail, Banner, sponsorid, setuppersonid, setuppersonid2, signuptype, orgtype, demo, email, website, facebook, twitter, leader_pic, location_pic,  group_team_pic, userName, banner_path, firstPass)";
		$query15 .= " VALUES('$dealer','$dealerdir','$about','$reason','$goal','$start','$endz', '$address1', '$address2', '$ct','$st','$zp','$payMail','$banner', '$sponsor', '$club_id','$repID', '$signupType', '$orgType', '$foundUser', '$email','$url','$face', '$twt','$leaderPic', '$locationPic', '$groupPic', '$email', '$banner','$pass')";
	     $res15 = mysqli_query($link, $query15)or die ("couldn't execute query 15x.".mysql_error($link));
	       ...
	       //get Dealer ID of user just inserted
		        ...
		        //insert into the orgMembers table
		        $queryH = "INSERT INTO orgMembers(Title,orgFName, orgLName, orgEmail, orgPhone,orgRel, fundraiserID, fund_owner_id, repID, isLeader)VALUES('$tt','$fname','$lname','$email','$phn', '$role', '$logID','$club_id', '$repID', '1')";
		        $resultH = mysqli_query($link, $queryH)or die ("couldn't execute query hx.".mysql_error($link));
	...
	$resultL = mysqli_query($link, $leaderInsert)or die ("couldn't execute Leader Insert.".mysql_error($link));
	/*
    $subject = "Hello ".$fname." ".$lname." You Are Now a Member Of ".$dealer." ".$dealerdir." Fundraiser";
    $message = "Hey ".$fname."!";
    $message .= "<br />";
      ...
      <div id="content">
		...
		<form class="" action="addFundLeader.php" method="Post" id="myForm" name="myForm" onsubmit="return validate();" enctype="multipart/form-data">
			<div class="table">
				...
				<div class="row">
					<span id="hd_gmfr5">Fundraiser Account:</span>
					<!--<span id="hd_fg4">Group:</span>-->
				</div> <!-- end row -->
