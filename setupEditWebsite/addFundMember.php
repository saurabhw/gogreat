<?php
      ...
      if(!isset($_SESSION['authenticated']) || $_SESSION['role'] != "RP")
       ...
       $getSC = "SELECT * FROM distributors WHERE email = '$myEmail' AND role = 'RP'";
	  ...
	  $club_type = $group_row['clubType'];
	  //$repID = $_SESSION['userId'];
          $sponsor = $group_row['sponsorid'];
	  $com = 0.35;
	  $user_page = "../membersite.php?group=";
		...
		//insert info
		$query2 = "INSERT INTO $table1 (companyName, FName, MName, LName, ssn, address1, address2,  city, state, zip, email, homePhone, fbPage, twitter, salesPerson, cellPhone, userPaypal, role, Dealer, userBaseCommPct, title, gender)";
		
		$query2 .= " VALUES('$dealer','$fname','$mname','$lname','$ssn','$ad1','$ad2',  '$city','$state','$zip','$email','$phn','$face','$twt', '$userID','$phn',  '$payMail','$role', '$dealerdir', '$com', '$tt', '$gd')";
		
       //insert Dealer
        $query3 = "INSERT INTO $table3 (Dealer, DealerDir, About, FundraiserReasons, FundraiserGoal, FundraiserStartDate, FundraiserEndDate, Address1, Address2, City, State, Zip, PaypalEmail, Banner, setuppersonid, setuppersonid2, goal2, setuppersonid3, signuptype, orgtype, email, website, facebook, twitter, leader_pic, location_pic,  group_team_pic, userName, banner_path, firstPass, clubType)";
        
		$query3 .= " VALUES('$dealer','$dealerdir','$about','$reason','$goal','$start','$endz', '$address1', '$address2', '$ct','$st','$zp','$payMail','$banner', '$club_id', '$userID', '$goal2','$sponsor', '$signupType', '$orgType', '$email','$url','$face', '$twt','$leaderPic', '$locationPic', '$groupPic', '$email', '$banner','$pass', '$club_type')";
		...
		if($res1 && $res2 && $res3){
			...
			mysqli_query($link, $query10);
		        ...
		        $queryH = "INSERT INTO orgMembers(Title,orgFName, orgLName, orgEmail, orgPhone,orgRel, fundraiserID, fund_owner_id, repID, scID, vpID)VALUES('$tt', '$fname','$lname','$email','$phn', '$role', '$logID','$club_id', '$userID', '$scID', '$vpID')";
    ...
    */
	  ...
	  else
	  {
	     ...
	     $foundPic = $userRow['picPath'];
	      ...
	      $query15 =  "INSERT INTO $table3 (Dealer, DealerDir, About, FundraiserReasons, FundraiserGoal, FundraiserStartDate, FundraiserEndDate, Address1, Address2, City, State, Zip, PaypalEmail, Banner, setuppersonid, setuppersonid2, goal2, setuppersonid3, signuptype, orgtype, demo, email, website, facebook, twitter, leader_pic, location_pic,  group_team_pic, contact_pic, userName, banner_path, firstPass, clubType)";
		$query15 .= " VALUES('$dealer','$dealerdir','$about','$reason','$goal','$start','$endz', '$address1', '$address2', '$ct','$st','$zp','$payMail','$banner', '$club_id','$userID', '$goal2', '$sponsor','$signupType', '$orgType', '$foundUser', '$email','$url','$face', '$twt','$leaderPic', '$locationPic', '$groupPic', '$foundPic', '$email', '$banner','$pass', '$club_type')";
	     $res15 = mysqli_query($link, $query15)or die ("couldn't execute query 15xg.".mysqli_error($link));
...
<div id="container">
      ...
      <div id="content">
		...
		<div class="table">
			<form class="" action="addFundMember.php" method="Post" id="myForm" name="myForm" onsubmit="return validate();" enctype="multipart/form-data">
				...
				<div class="row">
					...
					<div class="row">
						<select class="" name="groupid" id="groupid" onchange="fetch_select(this.value);" required>
							<option value="">Select Fundraiser Account</option>
							<?php 
						$getAccount = "SELECT * FROM Dealers WHERE setuppersonid = '$repID'";
				...
				<div class="simpleTabs">
					...
					<div class="interim-form">
						...
						<div id="row"> 
							...
							<input id="li" type="text" name="li" value="" placeholder="www.linkedin.com">
						</div> <!-- end row -->
						<!--<div id="row"> 
			...
			<form class="graybackground">
				...
				<input class="redbutton" type="submit" name="submit" value="Upload File">
			</form>-->
		</div> <!-- end table -->
		<br>
