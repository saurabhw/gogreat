<?php
   ...
   { 
     ...
     while($row = mysqli_fetch_assoc($result))
     {
       ...
       echo'<tr><td>'.$row['title'].'</td><td>'.$row['first'].'</td><td>'.$row['last'].'</td><td>'.$row['workPhone'].'</td><td>'.$row['email'].'</td><td>'.$row['relationship'].'</td><td>
                                        <a href="update.php?mi='.$row['fundMemberID'].'&id='.$row['customerID'].'"><input class="redbutton" type="button" value="Edit Contact" /></a>
