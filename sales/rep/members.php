...
$sql="SELECT * FROM Dealers where loginid='$groupid'";
...
while($arraycount>$x){
    ...
    if($lnames[$x]!=null && $emails[$x]!=null){
		...
		$query4 .= "VALUES('$dealer', '$dealerDir', '$about', '$f_r', '$go', '$start', '$ending',  '$a1', '$a2', '$ct', '$st', '$zp', '$pay', '$groupid', '$emails[$x]','$emails[$x]', '$banner', '$leaderpic', '$locationpic', '$groupteampic', '$contactpic', '$firstPass')";
		...
		$emailsql = "SELECT * FROM orgMembers WHERE fund_owner_id ='$repid' AND orgEmail='$emails[$x]' AND fundraiserID = '$groupid';";
                ...
                if(mysqli_num_rows($emailresult) < 1){
                   ...
                   $insert_result= mysqli_query($link, "INSERT INTO orgMembers (orgFName, orgLName, orgEmail, orgRel, orgPhone, fundraiserID,
               			    fund_owner_id, org_member_created)
                                    VALUES('$fnames[$x]', '$lnames[$x]', '$emails[$x]', '$rel[$x]', '$phone[$x]',
                                    '$user_infoIDX', '$groupid', now())") or die(mysqli_error());
		...
		if( $res1 && $res2 && $insert_result && $res4){
			...
			$group = $_GET['groupid'];
			//header( 'Location: editMembers.php?groupid=$group ') ;
			$redirect = "Location:editMembers.php?groupid=$group";
  	                header("$redirect"); 
...
<head>
<meta charset="UTF-8" />
<title>GreatMoods | Members</title>
...
<div id="container">
   ...
   <!--  <div class="distributor1">
     <table id="contacts">
      <tr>
          <td><label for="FName1"><b>First</b></label></td>
          <td><label for="LName1"><b>Last</b></label></td>
          <td><label for="title1"><b>Position Title</b></label></td>
          <td><label for="email1"><b>Email</b></label></td>
          <td><label for="phone1"><b>Phone</b></label></td>
    ...
    <div id="bannertable">
    <form id="graybackground5" action="" method="POST" name="fundraisercontacts" enctype="multipart/form-data" onSubmit="return validate(this);">
	<div id="row">
              ...
              <span id="loginemail">Email<span class="required">*</span></span>
              <span id="phone">Phone</span>
         </div> <!-- end row -->
	<div id="row">
              <input id="fname" type="text" name="fnames[]"/>
              <input id="lname" type="text" name="lnames[]"/>
              <input id="title" type="text" name="relation[]"/>
              <input id="loginemail" type="text" name="emails[]"/>
	...
	<div id="row">
          ...
          <input id="redbutton" type="submit" name="submit" value="Save New Member(s)" />
          <a id="redbutton" href="editMembers.php?groupid=<? echo $_GET['groupid']; ?>">Back to Members</a>
        </form>
