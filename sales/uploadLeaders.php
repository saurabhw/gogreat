     ...
     while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
      {
        ...
        $password1 = mysqli_real_escape_string($link, $filesop[4]);
		...
		$query3 .= "VALUES('$dealer','$dealerdir','$about','$reason','$goal','$start','$endz', '$phone1', '$address1', '$address2', '$ct','$st','$zp','$payMail','$banner', '$groupID','$repID','$sponsor', '$signupType', '$orgType', '$email','$url','$face', '$twt','$leaderPic', '$locationPic', '$groupPic', '$email','$banner', '$password1', '$club')";
		$result3 = mysqli_query($link, $query3)or die ("couldn't execute query 3.".mysqli_error($link));
		 
		 //get the member
		$queryL = "SELECT * FROM Dealers WHERE setuppersonid = '$groupID' AND userName = '$email'";
		$resultL = mysqli_query($link, $queryL)or die ("couldn't execute query L.".mysqli_error($link));
		$rowL = mysqli_fetch_assoc($resultL);
		$logID = $rowL['loginid'];
		        
		        //insert into the orgMembers table
  $queryH = "INSERT INTO orgMembers(Title, orgFName, orgLName, orgEmail, orgPhone,orgRel, fundraiserID, fund_owner_id, repID, isLeader, scID)VALUES('$titlex', '$fname','$lname','$email','$phone', '$role', '$logID','$groupID','$repID','1' ,'$userID')";
	  ...
	  $query2 .= "VALUES('$dealer', '$fname','$lname','$email', '$repID', '$phn', '$payMail','$role', '$dealerdir', '$titlex', '$user_table_id')";
        ...
        */
		...
		$logID = $rowL['loginid'];
		  ...
		  //insert into the orgMembers table
		$queryX = "INSERT INTO orgMembers(Title, orgFName, orgLName, orgEmail, orgPhone,orgRel, fundraiserID, fund_owner_id, repID, isLeader, scID)VALUES('$titlex', '$fname','$lname','$email','$phone', '$role', '$logID','$groupID','$repID', '1','$userID')";
