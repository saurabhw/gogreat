<?php
       ...
       //insert Dealer
		...
		if($res1 && $res2 && $res3){
		    ...
		    $queryx = "UPDATE Dealers SET demo = '$newUserID ', contact_pic='$personalPicPath' WHERE email = '$email'";      
		       $resultA = mysqli_query($link, $queryx)or die ("couldn't execute query x.".mysql_error()); 
			...
			mysqli_query($link, $query10);
		        ...
		        $resultL = mysqli_query($link, $queryL)or die ("couldn't execute query L.".mysql_error());
		        $rowL = mysqli_fetch_assoc($resultL);
		        $logID = $rowL['loginid'];
		        
		        //insert into the orgMembers table
		        $queryH = "INSERT INTO orgMembers(Title,orgFName, orgLName, orgEmail, orgPhone,orgRel, fundraiserID, fund_owner_id, repID)VALUES('$tt', '$fname','$lname','$email','$phn', '$role', '$logID','$club_id', '$repID')";
		        $resultH = mysqli_query($link, $queryH)or die ("couldn't execute query h.".mysql_error());
    ...
    */
	  ...
	  else
	  {
	     ...
	     $userRow = mysqli_fetch_assoc($user_result)or die ("couldn't execute query Get User.".mysql_error());
		...
		$query15 .= " VALUES('$dealer','$dealerdir','$about','$reason','$goal','$start','$endz', '$address1', '$address2', '$ct','$st','$zp','$payMail','$banner','$club_id','$repID', '$signupType', '$orgType', '$foundUser', '$email','$url','$face', '$twt','$leaderPic', '$locationPic', '$groupPic', '$foundPic', '$email', '$banner','$pass')";
	     $res15 = mysqli_query($link, $query15)or die ("couldn't execute query 15x.".mysql_error());
	     
	       //get Dealer ID of user just inserted
		        $queryL = "SELECT * FROM Dealers WHERE setuppersonid = '$club_id' AND email = '$email'";
		        $resultL = mysqli_query($link, $queryL)or die ("couldn't execute query Lx.".mysql_error());
		        $rowL = mysqli_fetch_assoc($resultL);
		        $logID = $rowL['loginid'];
      ...
      <div id="content">
		...
		<div class="table">
			<form class="" action="addFundMember.php" method="Post" id="myForm" name="myForm" onsubmit="return validate();" enctype="multipart/form-data">
				...
				<div class="row">
				<span id="hd_gmfr4">Select Representative:</span>
