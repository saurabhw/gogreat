...
function getRepTotal($rep) {
	...
	$sqlResult = mysqli_query($link, $sql)or die ("couldn't execute query sales.".mysql_error());
...
function getGroupSales($group) {
        ...
        global $link;
	$query = "SELECT SUM(Amount) as total FROM IPNdata  WHERE Rep = '$group'";
	$result = mysqli_query($link, $query)or die ("couldn't execute query sales.".mysql_error());
	$row = mysqli_fetch_assoc($result);
	$total = $row['total'];
	
	$getMembers = "SELECT * FROM orgMembers WHERE fund_owner_id = '$group'";
	$memberResult = mysqli_query($link, $getMembers) or die ("couldn't execute query members.".mysql_error());

	while($row2 = mysqli_fetch_assoc($memberResult))
	{
	   $memberID = $row2['fundraiserID'];
	   $membersSales = "SELECT SUM(Amount) as mt FROM IPNdata  WHERE Rep = '$memberID'";
	   $memberSalesResult = mysqli_query($link, $membersSales) 
	   or die ("couldn't execute query members.".mysql_error());
...
function getSoloSales($group) {
	...
	$getMembers = "SELECT SUM(Amount) as total FROM IPNdata WHERE Rep = '$group'";
	$memberResult = mysqli_query($link, $getMembers) or die ("couldn't execute query members.".mysql_error());
