...
<div id="container">
	...
	<div id="content">
		...
		<h1>Member Team & Accounts</h1>
		<h3>Current Friends, Family and Business Accounts</h3>
		...
		<div>
			...
			<table id="ff_accts">
				<tr>
					...
					<th><label for="last">Address</label></th>
					<th><label for="role">Relationship</label></th>
				...
				$customers_query = "SELECT * FROM orgCustomers WHERE fundMemberID = '$dealerID'";
				...
				if ($customers_result) {
					while ($row_Info = mysqli_fetch_assoc($customers_result)) {
						...
						$edit_page = "";
						$conType = htmlspecialchars($row_Info['relationship']);
						$id = htmlspecialchars($row_Info['customerID']);
						switch($conType)
						{
						   case "Business Supporter":
						   $edit_page = "editBusinessSupporter.php?id=$id";
						   break;
						   
						   case "Business Associate":
						   $edit_page = "editBusinessAssociate.php?id=$id";
						   break;
						   
						   case "Friends Family":
						   $edit_page = "editFriend.php?id=$id";
						   break;
						}
						...
						<tr>
							...
							<td class="cn">' . htmlspecialchars($row_Info['companyname']) . '</td>
							<td class="title">' . htmlspecialchars($row_Info['title']) . '</td>
							<td class="fn">' . htmlspecialchars($row_Info['first']) . '</td>
							<td class="ln">' . htmlspecialchars($row_Info['last']) . '</td>
							<td class="address">
						' . htmlspecialchars($row_Info['address']) . '
						' . htmlspecialchars($row_Info['apt']) . '
								' . htmlspecialchars($row_Info['city']) . '
								' . htmlspecialchars($row_Info['state']) . '
							</td>
							<td class="role">' . htmlspecialchars($row_Info['relationship']) . '</td>
							<td class="email">' . htmlspecialchars($row_Info['email']) . '</td>';
							
								if ($row_Info['mobile'] == NULL) {
						echo '<td class="pph">'.htmlspecialchars($row_Info[workPhone]) . '</td>';
									...
									echo '<input type="text" name="update_home" class="pph" value="' . htmlspecialchars($row_Info['homePhone']) . '"/>';
								}
								else {
									echo '<input type="text" name="update_mobile" class="pph" value="' . htmlspecialchars($row_Info['mobile']) . '"/>';
								}
						echo '</td>
							
							<td class="actions">
								<a href="editContact.php?con='.htmlspecialchars($row_Info['customerID']).'" ><input class="redbutton" type="button" value="Edit Contact" /></a>
								<a href="deleteCustomer.php?cust='.htmlspecialchars($row_Info['customerID']).'&group='.$dealerID.'"><input class="redbutton" type="button" value="Delete" /></a>
								<a href="emails2.php"><input class="redbutton" type="button" value="Send Emails" /></a>
						...
						// END FORM FOR TABLE [orgCustomers] - THIS ALLOWS USERS TO UPDATE THEIR CONTACTS ON THIS PAGE
							...
							//<td>
								//<form action="customer_delete.php?group=' . urlencode($_GET["group"]) . '" method="POST" enctype="multipart/form-data">
									...
									//<input type="submit" name="delete" value="X" class="edit_contact, redbutton" title="Delete Contact"/></span>
								//</form>
							//</td>
						 echo '</tr>';
		...
		<!--<div>
			...
			<!--<div id="">--> <!--THIS DIV IS HERE TO SIMPLY ADD THE BACKGROUND COLOR & PADDING-->
			<!--<form action="customer_add.php" method="POST" name="addnewcontact" enctype="multipart/form-data" onsubmit="return validateAddNewContact()">
				...
				<table>-->
					...
					<tr>-->
						...
						<td><select name="update_relationship" style="width:107px;">
						...
						<td>
							<label>
								<select name="gender" style="width:107px;">
