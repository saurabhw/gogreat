     ...
     while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
      {
        ...
        if(mysqli_num_rows($resultB) > 0)
        {
	     ...
	     $dealer = addslashes($dealer);
	      ...
	      $query3 = "INSERT INTO Dealers (Dealer, DealerDir, About, FundraiserReasons, FundraiserGoal,  FundraiserStartDate, FundraiserEndDate, Phone, Address1, Address2, City, State, Zip, PaypalEmail, Banner,  setuppersonid, setuppersonid3, setuppersonid2, signuptype, orgtype, email, website, facebook, twitter, leader_pic, location_pic,  group_team_pic, userName, banner_path, firstPass, clubType)";
		$query3 .= "VALUES('$dealer','$dealerdir','$about','$reason','$goal','$start','$endz', '$phone1', '$address1', '$address2', '$ct','$st','$zp','$payMail','$banner', '$groupID','$repID','$sponsor', '$signupType', '$orgType', '$email','$url','$face', '$twt','$leaderPic', '$locationPic', '$groupPic', '$email','$banner', '$password', '$club')";
	...
	$resultH = mysqli_query($link, $queryH)or die ("couldn't execute query h.".mysqli_error($link));
	  ...
	  $query2 .= "VALUES('$fname','$lname','$email','$phn', '$payMail','$role', '$title')";
        ...
        */
		...
		$query3 .= " VALUES('$dealer','$dealerdir','$about','$reason','$goal','$start','$endz', '$phone1', '$address1', '$address2', '$ct','$st','$zp','$payMail','$banner',  '$groupID','$repID','$sponsor', '$signupType', '$orgType', '$email','$url','$face', '$twt','$leaderPic', '$locationPic', '$groupPic', '$email','$banner','$pass', '$club')";
		 $queryResult3 = mysqli_query($link, $query3)or die ("couldn't execute query 3.".mysqli_error());
		 //get the member
		...
		$logID = $rowL['loginid'];
		  ...
		  //insert into the orgMembers table
		$queryX = "INSERT INTO orgMembers(Title, orgFName, orgLName, orgEmail, orgPhone,orgRel, fundraiserID, fund_owner_id, repID, isLeader)VALUES('$titlex', '$fname','$lname','$email','$phone', '$role', '$logID','$groupID','$userID', '1')";
