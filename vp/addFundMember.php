...
$userID = $_SESSION['userId'];
	  ...
	  $phone = mysqli_real_escape_string($link, $group_row['Phone']); 
	  ...
	  $user_page = "../membersite.php?group=";
		...
		//insert info
		$query2 = "INSERT INTO $table1 (companyName, FName, MName, LName, ssn, address1, address2,  city, state, zip, email, homePhone, fbPage, twitter, salesPerson, cellPhone, workPhone, userPaypal, role, Dealer, userBaseCommPct, title, gender)";
		
		$query2 .= " VALUES('$dealer','$fname','$mname','$lname','$ssn','$ad1','$ad2',  '$city','$state','$zip','$email','$phn','$face','$twt', '$repID','$phn', '$phn', '$payMail','$role', '$dealerdir', '$com', '$tt', '$gd')";
       ...
       //insert Dealer
		...
		if($res1 && $res2 && $res3){
			...
			mysqli_query($link, $query10);
		        ...
		        $queryH = "INSERT INTO orgMembers(Title,orgFName, orgLName, orgEmail, orgPhone,orgRel, fundraiserID, fund_owner_id, repID, scID, vpID)VALUES('$tt', '$fname','$lname','$email','$phn', '$role', '$logID','$club_id', '$repID', '$scID', '$userID')";
		...
		<div class="table">
			<form class="" action="addFundMember.php" method="Post" id="myForm" name="myForm" onsubmit="return validate();" enctype="multipart/form-data">
				...
				<span id="hd_gmfr4">Fundraiser Account:</span>
					...
					<select class="" name="groupid" id="groupid" required>
						<option>Select</option>	
					</select>
					</div> <!-- end row -->
