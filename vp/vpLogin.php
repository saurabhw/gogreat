<?
      ...
      if(!isset($_SESSION['authenticated']) || $_SESSION['role'] != "VP")
       ...
       $resultx = mysqli_query($link, $query)or die ("couldn't execute query.".mysql_error());
       ...
       $getDis = "SELECT * FROM distributors WHERE email = '$userEmail' AND role = 'RP'";
       ...
       $getSC = "SELECT * FROM distributors WHERE email = '$userEmail' AND role = 'SC'";
       $result3 = mysqli_query($link, $getSC)or die ("couldn't execute get vp query.".mysqli_error($link));
...
<div id="container">
  ...
  <?
    if(mysqli_num_rows($result) > 0)
       ...
       {
         //echo 'user has RP account'; 
         echo '<form action="../setupEditWebsite/editClub.php" method="Post"><input type="submit" class="redbutton" value="Login as Representative" name="submit1"></form><br>';
         
       }
       else
       {
          //echo 'No RP account.<br>';
       }
      ...
      echo '<a href="accounts.php"><input type="button" class="redbutton" value="Login as Vice President" name=""></a><br>';
  ?>
   </div>  <!--end content-->
