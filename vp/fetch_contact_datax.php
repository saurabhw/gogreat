<?php
   ...
   { 
     ...
     $query = "Select * FROM orgCustomers  WHERE  fundMemberID='$vp'";
     $result = mysqli_query($link, $query)or die("MySQL ERROR on query c: ".mysqli_error($link));
     echo'<tr> 
                                <th class="fn" title="title">Company</th>
     ...
     while($row = mysqli_fetch_assoc($result))
     {
       ...
       echo'<tr class="odd"><td>'.$row['companyname'].'</td><td>'.$row['title'].'</td><td>'.$row['first'].'</td><td>'.$row['last'].'</td><td>'.$row['workPhone'].'</td><td>'.$row['email'].'</td><td>'.$row['relationship'].'</td><td>
