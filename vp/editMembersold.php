<?php
	...
	$pages_query = "SELECT COUNT('orgMemberID') FROM orgMembers where fund_owner_id='$group'";
		$many = mysqli_query($link, $pages_query)or die ("couldn't execute  pages query.".mysqli_error($link)); 
		$rowx = $many->fetch_row();
		$pages = ceil($rowx[0] / $per_page);
	$query = "SELECT * FROM orgMembers WHERE fund_owner_id='$group'  ORDER BY orgLName ASC LIMIT $start_from, $per_page";
...
<div id="container">
	...
	<div id="content">
	   ...
	   $queryS = "SELECT SUM(Amount) as total FROM IPNdata WHERE Rep='$member'"; 
           $resultS = mysqli_query($link, $queryS) or die("No IPN Data.".mysqli_error($link));
            // Print out result
           $rowS = mysqli_fetch_assoc($resultS);
	   $amount =  $rowS['total'];
						echo'<tr>
							<td class="title"><input name="" type="text" value="'.$row['Title'].'" readonly></td>
							<td class="fn"><input name="" type="text" value="'.$row['orgFName'].'" readonly></td>
							<td class="ln"><input name="" type="text" value="'.$row['orgLName'].'" readonly></td>
							<td class="role"><input name="" type="text" value="'.$row['orgRel'].'" readonly></td>
							<td class="email"><input name="" type="text" value="'.$row['orgEmail'].'" readonly></td>
							<td class="pph"><input name="" type="text" value="'.$row['orgPhone'].'" readonly></td>
							...
							<td class="actions">
							<a href="memberEdit.php?group='.$row['fundraiserID'].'"><input class="redbutton" type="button" value="Edit Member" /></a>
							...
							echo'<form class="buttonform" method="post" action="deleteMember.php">
								<input type="hidden" name="clubid" value="'.$row['fundraiserID'].'" />
				...
				<?
					if($pages >= 1 && $page <= $pages){
						for($x = 1; $x <= $pages; $x++){
							echo'<a href=?group='.$group.'&page='.$x.'>'.$x.'</a> ';
						}
					}
				?>
