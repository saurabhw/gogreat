<?php
   ...
   { 
			  ...
			  {
			     echo '<tr><td colspan="7">No Sales Recorded.</td></tr>';
			  }
			  while($memberRow = mysqli_fetch_assoc($salesMemberResult))
			  {
			     ...
			     $scCom = $memberRow[Amount] * .01;
			     $scCom = round($scCom, 2);
			     $myCom = $memberRow[Amount] * .005;
			     $myCom = round($myCom, 2);
			     echo '<tr>
			...
			echo' <tr class="total">
				...
				$memberTotal = $rowT4['total'] * .35;
			        ...
			        $scTotal = $rowT4['total'] * .01;
			        $scTotal = round($scTotal, 2);
			        $myTotal = $rowT4['total'] * .005;
				...
				<td class="amt" colspan="1">$'.$scTotal.'</td>
